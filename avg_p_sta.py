...
# 计算多个被试的平均数据
# Calculate average power for each raw object in data_list
...
li_fre=[(0.1, 3),(4, 7),(8, 12),(13, 29),(30, 100)]
result=[]
for freband in li_fre:
    raw_combined.filter(*freband, picks='eeg')
    result.append(grand_average_data)
print(result)
matrix_list=result
...
# 将DataFrame写入Excel文件
excel_path = r"D:\mnepythonEEG\datapower1.xlsx"  # 替换成你想要保存的Excel文件路径
...
freq_band_power = np.mean(raw_combined.get_data())  # 这里计算的是平均功率
data_list.append(freq_band_power)
print(data_list)

# 计算频段内的平均功率或能量
freq_band_power = np.mean(np.abs(raw_combined._data) ** 2, axis=-1)  # 这里计算的是平均功率
data_list.append(freq_band_power)
print(data_list)
